<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('catalog.catalogs.edit', $catalog) }}
    @endsection
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="editCatalogForm" action="{{ route('catalog.catalogs.update', $catalog) }}" method="POST"
                    ...
                    <div class="py-4 card card-flush">
                        ...
                        <div class="pt-0 text-center card-body">
                            ...
                            <div class="text-muted fs-7">
                                Set the catalog thumbnail image. Only *.png, *.jpg and *.jpeg image files are accepted
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="py-4 card card-flush">
                        ...
                        <div class="pt-0 card-body">
                            <select class="form-select" name="is_active" id="isActive" data-control="select2" required>
                                <option value="1"
                                    {{ old('is_active', $catalog->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0"
                                    {{ old('is_active', $catalog->is_active) == 0 ? 'selected' : '' }}>Inactive
                                </option>
                            </select>
                            ...
                            <select class="form-select" name="related_product_id" id="related_product_id"
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}"
                                        ...
                                        {{ old('related_product_id', $catalog->related_product_id) == $product->id ? 'selected' : '' }}>
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="text-muted fs-7">Optionally associate this catalog with a product.</div>
                            <script>
                    ...
                    <div class="py-4 card card-flush">
                        ...
                        <div class="pt-0 card-body">
                            <input type="number" name="sort_order"
                                ...
                                value="{{ old('sort_order', $catalog->sort_order ?? 0) }}" />
                            @error('sort_order')
                    ...
                    <div class="mb-1 row">
                        <div class="col-lg-6">
                            ...
                            <input type="text" name="name"
                                ...
                                placeholder="Enter catalog name" value="{{ old('name', $catalog->name) }}" required />
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4"
                                placeholder="Enter catalog description">{{ old('description', $catalog->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="pt-0 card-body">
                            <div class="mb-10">
                                ...
                                <input type="text" class="mb-2 form-control" id="metaTitle" name="meta_title"
                                    ...
                                    value="{{ old('meta_title', $catalog->meta_title) }}" />
                                <div class="text-muted fs-7">Set a meta tag title. Recommended to be simple
                                    and precise keywords.</div>
                            </div>
                            <div class="mb-10">
                                <label class="form-label">Meta Tag Description</label>
                                <textarea id="metaDescriptionEditor" name="meta_description" class="mb-2 form-control" style="min-height: 100px;">{{ old('meta_description', $catalog->meta_description) }}</textarea>
                                <div class="text-muted fs-7">Set a meta tag description to the catalog for
                            ...
                            <div>
                                ...
                                <input id="kt_ecommerce_add_product_meta_keywords" name="meta_keywords"
                                    ...
                                    value="{{ old('meta_keywords', $catalog->meta_keywords) }}" />
                                <div class="text-muted fs-7">Set a list of keywords that the catalog is
                                    related to. Separate the keywords by adding a comma <code>,</code> between each
                                    keyword.</div>
                            </div>
                        </div>
                    </div>
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('catalog.catalogs.index') }}" class="btn btn-light me-3">Cancel</a>
                        <button type="submit" class="btn btn-primary" id="submitBtn">
                            <span class="indicator-label">Update Catalog</span>
