            ...
            <div class="card-title">
                <div class="my-1 d-flex align-items-center position-relative">
                    ...
                    <input type="text" class="form-control form-control-solid w-250px ps-13"
                        placeholder="Search category" id="searchInput" />
                </div>
            </div>

            <div class="card-toolbar">
                <div class="d-flex justify-content-end align-items-center">
                    <select class="form-select form-select-solid w-200px me-3" id="catalogFilter"
                        data-control="select2">
                        <option value="">All Catalogs</option>
                        @foreach ($catalogs as $catalog)
                            <option value="{{ $catalog->id }}"
                                {{ request('catalog_id') == $catalog->id ? 'selected' : '' }}>
                                {{ $catalog->name }}
                            </option>
                        @endforeach
                    </select>

                    <a href="{{ route('catalog.categories.create') }}" class="btn btn-primary">
                        {!! getIcon('plus', 'fs-2', '', 'i') !!}
                        Add Category
                    </a>
                </div>
                    ...
                    <tbody class="text-gray-600 fw-semibold">
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('catalog.catalogs.show', $category->catalog) }}"
    ...
    @push('scripts')
        ...
        <script>
            ...
            document.getElementById('searchInput').addEventListener('keyup', function() {
                ...
                let tableRows = document.querySelectorAll('#categoriesTable tbody tr');
